...
| `chat-jump-to-latest.mjs` | **채팅 "최근 메시지로 이동" 버튼 + auto-stick**: 대량 전송 후 맨 아래 붙음, 위로 스크롤 시 버튼 노출, 클릭 시 맨 아래 복귀, **위로 스크롤 중 원격 메시지가 와도 위치 유지 + "새 메시지 N개" 배지**(핵심 회귀). | 2 | `node e2e/chat-jump-to-latest.mjs` |
